        ...
        if(\Auth::user()->can('create client'))
        {
            ...
            if($total_client < $plan->max_clients || $plan->max_clients == -1)
            {
                ...
                if($setings['new_client'] == 1)
                {
                    ...
                    return redirect()->route('clients.index')->with('showProjectModal', true)->with('success', __('Client successfully added.') . ((!empty($resp) && $resp['is_success'] == false && !empty($resp['error'])) ? '<br> <span class="text-danger">' . $resp['error'] . '</span>' : ''));
                }
                return redirect()->route('clients.index')->with('showProjectModal', true)->with('success', __('Client successfully created.'));
            }
            else
            {
