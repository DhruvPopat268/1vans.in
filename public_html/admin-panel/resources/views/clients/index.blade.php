...
<div class="row">
                                ...
                                <div class="dropdown-menu icon-dropdown dropdown-menu-end">
                                    @can('edit client')
                                        <a href="#!" data-size="md"
                                            ...
                                            class="dropdown-item" data-bs-original-title="{{ __('Edit Project Client') }}">
    ...
    <div class="col-xl-3 col-lg-4 col-sm-6 mb-4">
        <a href="#" data-size="md" data-url="{{ route('clients.create') }}" data-ajax-popup="true"
            data-bs-toggle="tooltip" data-bs-original-title="{{ __('Create New Project Client') }}"
            class="btn-addnew-project border-primary">
            <div class="bg-primary proj-add-icon">
                <i class="ti ti-plus"></i>
            </div>
            <h6 class="mt-3 mb-2">{{ __('Create Project Client') }}</h6>
            <p class="text-muted text-center mb-0">{{ __('Click here to add new Project Client') }}</p>
        </a>
    </div>
</div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ProjectModalLabel">{{ __('Add Project') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <p>{{ __('Would you like to add a Project now?') }}</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('No') }}</button>
                    <button type="button" class="btn btn-primary" onclick="handleProjectChoice('yes')">{{ __('Yes') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function handleProjectChoice(choice) {
                if (choice === 'yes') {
                    window.location.href = "{{ route('projects.index') }}"; // Redirect to the Project index page
                }
            }
    </script>
@endpush
