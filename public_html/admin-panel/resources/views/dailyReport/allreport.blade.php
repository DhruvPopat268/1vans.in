...
@push('script-page')
<script>
    $('#wing_id').on('change', function () {
        var wingId = $(this).val();
        var flourDropdown = $('#flour_id');
        flourDropdown.empty().append('<option value="">{{ __('All') }}</option>');

        if (wingId) {
            $.ajax({
                url: "{{ url('/get-flours-by-wing') }}/" + wingId,
                type: 'GET',
                success: function (res) {
                    $.each(res, function (key, value) {
                        flourDropdown.append('<option value="' + value.id + '">' + value.name + '</option>');
                    });
                }
            });
        }
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{__('Daily All Report')}}</li>
@endsection
@section('action-btn')
    <div class="float-end">
    </div>
...
<div class="row">
    <div class="row" style="margin-bottom: 10px;margin-top:10px;">
            <div class="col-12">
                <!-- Filter Form -->
 <form method="GET" action="{{ route('all-report.index') }}" class="row mb-4">
        <div class="col-md-3">
            <label>{{ __('Name of Work') }}</label>
            <select name="name_of_work_id" class="form-control">
                <option value="">{{ __('All') }}</option>
                @foreach($nameOfWorks as $work)
                    <option value="{{ $work->id }}" {{ request('name_of_work_id') == $work->id ? 'selected' : '' }}>
                        {{ $work->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-3">
            <label>{{ __('Work Section') }}</label>
            <select name="flour_id" id="flour_id" class="form-control">
                <option value="">{{ __('All') }}</option>
                @foreach($flours as $flour)
                    <option value="{{ $flour->id }}" {{ request('flour_id') == $flour->id ? 'selected' : '' }}>
                        {{ $flour->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-3 align-self-end">
            <button type="submit" class="btn btn-primary">{{ __('Filter') }}</button>
                    <a href="{{ route('all-report.index') }}" class="btn btn-secondary ms-2">{{ __('Reset') }}</a>
                    ...
                    <table class="table datatable">
                        <thead>
                            <tr>
                                ...
                                <!--<th>{{ __('Work Section') }}</th>-->
                                <th>{{ __('Location') }}</th>
                                <th>{{ __('Working Area') }}</th>
                                <th>{{ __('Man Power') }}</th>
                            ...
                            @foreach($dailyreport as $report)
                                <tr>
                                    ...
                                    <td>{{ $report->location ?? '-' }}</td>
                                      <td>{{ $report->wing->name ?? '-' }} ({{ $report->flour->name ?? '-'  }})</td>
                                    <td>
    <ul>
        @php
            $manPowerGrouped = [];
