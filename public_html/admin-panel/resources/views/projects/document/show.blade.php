...
@section('page-title')
    {{__('Manage Project Document Attachments')}}
@endsection
@push('css-page')
...
<script>
    ...
    myDropzone = new Dropzone("#dropzonewidget", {
       ...
       url: "{{route('ProjectDocument.file.upload',[$ProjectDocument->id])}}",
    ...
    myDropzone.on("sending", function (file, xhr, formData) {
        ...
        formData.append("contract_id", {{$ProjectDocument->id}});
    });
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{__('Project Document Attachments')}}</li>
@endsection
@section('action-btn')
@endsection
        ...
        <div id="useradd-2">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">{{ __('Project Document Attachments') }}</h5>
                </div>
                <div class="card-body">
...
@foreach($ProjectDocument->attachments as $file)
                        <div class="card mb-3 border shadow-none">
                           <div class="px-3 py-3">
                              <div class="row align-items-center">
                                 <div class="col">
                                    <h6 class="text-sm mb-0">
                    <a href="{{ asset('storage/project_document_attachment/' . $file->files) }}" target="_blank">
                        {{ basename($file->files) }}
                    </a>
                                    </h6>
                                    <p class="card-text small text-muted">
                    @if(!empty($file->files) && file_exists(storage_path('project_document_attachment' . $file->files)))
                        {{ number_format(\File::size(storage_path('project_document_attachment' . $file->files)) / 1048576, 2) . ' MB' }}
                                       @endif
                                    </p>
                                 </div>
                ...
                <div class="action-btn bg-warning">
                    <a href="{{ asset('storage/project_document_attachment/' . $file->files) }}"
                       ...
                       data-bs-toggle="tooltip" title="Download">
                        <span class="text-white"><i class="ti ti-download"></i></span>
                                  ...
                                  @can('delete site documents')
                                    <div class="action-btn bg-danger">
                    {!! Form::open(['method' => 'DELETE', 'route' => ['ProjectDocument.attachment.delete', $file->id]]) !!}
