...
@php
    $profile=\App\Models\Utility::get_file('uploads/avatar/');
        $pdfLogo=\App\Models\Utility::get_file('uploads/pdf_logo/');
...
@section('content')
    ...
    <div class="form-group mt-4">
        <div class="theme-avtar-logo mt-4">
                                        <img id="image" src="{{ ($userDetail->pdf_logo) ? $pdfLogo  . $userDetail->pdf_logo : $pdfLogo . 'avatar.png' }}"
                                             class="big-logo">
                                    </div>
        <label for="pdf_logo" class="col-form-label text-dark">{{ __('Upload PDF Logo') }}</label>
        <input type="file" name="pdf_logo" id="pdf_logo" class="form-control">
        <span class="text-xs text-muted">{{ __('Please upload a valid PDF file. Max size 2MB.') }}</span>
    </div>
</div>
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('update.project') }}">
                        ...
                        <div class="row">
                            <div class="col-lg-6 col-sm-6">
                                <div class="form-group">
                                    <label for="project_assign_id" class="col-form-label text-dark">{{ __('Project') }}</label>
                                    <select name="project_assign_id" id="project_assign_id" class="form-control">
                            ...
                            <div class="col-lg-12 text-end">
                                <input type="submit" value="{{ __('Update Project') }}" class="btn btn-primary m-r-10">
                            </div>
                        </div>
                    </form>
                </div>
