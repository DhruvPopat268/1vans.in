...
@section('breadcrumb')
    ...
    @else
        <li class="breadcrumb-item">{{ __('Project Engineers') }}</li>
    @endif
@endsection
@section('action-btn')
    <div class="float-end">
        @if (\Auth::user()->type == 'company' || \Auth::user()->type == 'HR')
            ...
            <!--    data-bs-toggle="tooltip" data-bs-placement="top" title="{{ __('Project Engineers Logs History') }}"><i-->
            <!--        class="ti ti-user-check"></i>-->
            <!--</a>-->
              <a href="#" data-size="md"  data-bs-toggle="tooltip" title="{{__('Import')}}" data-url="{{ route('user.file.import') }}" data-ajax-popup="true" data-title="{{__('Import User CSV file')}}" class="btn btn-sm btn-primary">
Import        </a>
        @endif
        @can('create user')
          <a href="#" 
   ...
   data-title="{{ \Auth::user()->type == 'super admin' ? __('Create Admin') : __('Create Project Engineer') }}" 
   data-bs-original-title="{{ \Auth::user()->type == 'super admin' ? __('Create Admin') : __('Create Project Engineer') }}" 
   class="btn btn-sm btn-primary me-1 d-inline-flex align-items-center gap-1">
    <i class="ti ti-plus"></i>
    <span>
        {{ \Auth::user()->type == 'super admin' ? __('Create Admin') : __('Create Project Engineer') }}
    </span>
</a>
...
<div class="row">
                                ...
                                <div class="dropdown-menu icon-dropdown dropdown-menu-end">
                                    @can('edit user')
                                        <a href="#!" data-size="lg" data-url="{{ route('users.edit', $user->id) }}"
                                            ...
                                            data-bs-original-title="{{ \Auth::user()->type == 'super admin' ? __('Edit Admin') : __('Edit Project Engineer') }}">
                                    ...
                                    @if (Auth::user()->type == 'super admin')
                                        <a href="{{ route('login.with.company', $user->id) }}" class="dropdown-item"
                                            data-bs-original-title="{{ __('Login As Admin') }}">
                ...
                @if (\Auth::user()->type == 'super admin')
                    <div class="btn-wrp d-flex align-items-center gap-2 border-bottom border-top py-3 my-3">
                        ...
                        <a href="#" data-url="{{ route('company.info', $user->id) }}" data-size="lg"
                            ...
                            data-title="{{ __('Admin Info') }}">{{ __('Admin Hub') }}</a>
                    </div>
                    ...
                    <div
                        class="user-count-wrp d-flex align-items-center justify-content-between gap-2">
                         <div class="user-count d-flex align-items-center gap-2" data-bs-toggle="tooltip"
                            ...
                            {{ $user->totalAppUser($user->id) }}
                        </div>
                         <div class="user-count d-flex align-items-center gap-2" data-bs-toggle="tooltip"
                            title="{{ __('Client') }}">
                            <div class="user-icon d-flex align-items-center justify-content-center">
                                 <i class="f-16 ti ti-briefcase text-white"></i>
                            </div>
                            {{ $user->totalClient($user->id) }}
                        </div>
                          ...
                          <div class="user-count d-flex align-items-center gap-2" data-bs-toggle="tooltip"
                            ...
                            {{ $user->totalProjectTeam($user->id) }}
                        </div>


                        <div class="user-count d-flex align-items-center gap-2" data-bs-toggle="tooltip"
                            title="{{ __('Project') }}">
                            <div class="user-icon d-flex align-items-center justify-content-center" style="background: #d13cc2">
                                <i class="f-16 ti ti-users text-white"></i>
                            </div>
    ...
    <div class="col-xxl-3 col-lg-4 col-sm-6 mb-4">
        <a href="#" class="btn-addnew-project border-primary" data-ajax-popup="true"
            data-url="{{ route('users.create') }}"
            data-title="{{ \Auth::user()->type == 'super admin' ? __('Create Admin') : __('Create Project Engineers') }}"
            data-bs-toggle="tooltip" title=""
            data-bs-original-title="{{ \Auth::user()->type == 'super admin' ? __('Create Admin') : __('Create Project Engineers') }}">
            <div class="bg-primary proj-add-icon">
                <i class="ti ti-plus"></i>
            </div>
            <h6 class="mt-3 mb-2">
                {{ \Auth::user()->type == 'super admin' ? __('Create Admin') : __('Create Project Engineers') }}</h6>
            <p class="text-muted text-center mb-0">
                {{ \Auth::user()->type == 'super admin' ? __('Click here to add new Admin') : __('Click here to add new Project Engineers') }}
            </p>
        </a>
    </div>
</div>
